...
Provenance note (not a substantive revision): this prediction was drafted on
2026-06-01 by the `draft-prediction` job under the previous weekly cadence,
which synthesized only the single latest digest (`signals/2026-W22.md`). As of
2026-06 the job runs monthly and synthesizes the whole prior month's weekly
digests (see PR #11). The frozen body above — falsifier, predicted shape, and
window — is unaffected and remains the operative test. Status stays `open`.
